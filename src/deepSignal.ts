...
import { alienSignal, computed } from './core';
import {
...
function setSignalValue(meta: ProxyMeta, key: PropertyKey, value: any) {
  ...
  if (!signals.has(key)) {
    ...
    for (const subscriber of existingSignal.externalSubscribers.values()) {
...
function setValToSignalRecord(record: SignalRecord, value: any) {
  ...
  record.externalSubscribers.forEach(subscriber =>
    subscriber.onSet(value),
  );
}
 ...
 * - Use user-provided propGenerator for synthetic ids and add returned extra properties
...
function assignSyntheticId(
  ...
  if (
    synthetic === undefined &&
    generatorValue?.syntheticId !== undefined
  ) {
...
const getArrayMutationProxy = (
  ...
  if (key === 'reverse') {
    ...
    return (target as any).toReversed;
  } else if (key === 'sort') {
    ...
    return (start: number, deleteCount: number, ...items: any[]) => {
      ...
      schedulePatch(meta, () => {
        ...
        for (const newItem of [...items].reverse()) {
          patches.push({
            op: 'add',
            path: buildPath(meta, String(start)),
            value: newItem,
...
export function deepSignal<T extends object>(
  ...
  if (isDeepSignal(input)) {
    ...
    const additional = specialOptions?.subscriberFactories ?? new Set();
    meta.options.subscriberFactories = new Set([
...
export function subscribeDeepMutations(
  ...
  const rootId =
    typeof root === 'symbol' ? root : getDeepSignalRootId(root);
  if (!rootId)
 ...
 * @example
 * ```ts
 * import { shallow } from "@bemedev/alien-deepsignals";
...
export function setSetEntrySyntheticId(obj: object, id: string | number) {
  ...
  // @ts-expect-error RAW_KEY is a symbol key not in the object type
